...
function createLikesSection(data) {
	...
	for (var b = 0; b < data.dislikers.length; b++) {
		var disliker = data.dislikers[b];
		...
		html += '<ul>';
			...
			html += '<li>';
				...
				html += '<span class="' + className + '" onclick="toggleDislikePost(this);" data-id="' + data._id + '">';
					html += '<i class="ti-thumb-down"></i>';
				html += '</span>';

				html += '<ins class="dislikers-count" data-id="' + data._id + '" onclick="showPostDislikers(this);">' + data.dislikers.length + '</ins>';
